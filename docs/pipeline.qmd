...
| RSeQC (all modules) | 1–4 h | TIN calculation slowest; BAMs above `rseqc_tin_downsample_reads` (default 50M reads, i.e. ~25M pairs) are downsampled first to bound runtime; set runtime to 36 h on Rivanna |
...
2. **RSeQC TIN:** Scales poorly with high coverage. BAMs above `rseqc_tin_downsample_reads` (default 50M primary-mapped reads — counted per mate, i.e. ~25M pairs for paired-end data) are randomly downsampled first (mates kept together, no singletons) to bound TIN's runtime. Increase `rseqc_tin_runtime` in Rivanna profile if still needed (default 36 h; usually sufficient)
