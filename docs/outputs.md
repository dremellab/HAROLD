...
| `counts/normalized_counts/{variant}/limma_pseudo_rpkm_counts.tsv` | **Limma/voom-normalized pseudo-RPKM matrix**, all genes. Built from the ERCC-corrected matrix above with limma's own modeling applied on top — not the plain ERCC-corrected values themselves; see `ERCC_corrected_log2_counts.tsv` for that. **Only produced for `w_ercc_*` variants.** |
| `counts/normalized_counts/{variant}/limma_log2normalized_pseudo_rpkm_counts.tsv` | Log2 of the matrix above. **Only produced for `w_ercc_*` variants.** |
| `counts/normalized_counts/{variant}/limma_log2normalized_pseudo_rpkm_counts_batch_corrected.tsv` | Same as the log2 matrix above, with `limma::removeBatchEffect` applied. **Only produced for `w_ercc_w_batch`** (`diffex.use_ercc: true`/`both` AND `diffex.use_batch: true`/`both`). |
| `counts/normalized_counts/{variant}/limma_voom_normalized_counts.tsv` | **Limma/voom-normalized matrix**, all genes — the `wo_ercc_*` counterpart to the pseudo-RPKM matrix above, written when no ERCC spike-in correction is applied. **Only produced for `wo_ercc_*` variants.** |
| `counts/normalized_counts/{variant}/limma_voom_normalized_counts_batch_corrected.tsv` | Same as the matrix above, with `limma::removeBatchEffect` applied. **Only produced for `wo_ercc_w_batch`.** |
...
> **Note:** `diffex normalize` also writes `edgeR_TMM_normalized_logCPM_counts_batch_corrected.tsv` and `DESeq2_vst_normalized_counts_batch_corrected.tsv` for `w_batch` variants, but these are not currently declared as tracked Snakemake outputs ([dremellab/HAROLD#68](https://github.com/dremellab/HAROLD/issues/68)) — they may be present in the output directory but aren't guaranteed or validated by the pipeline.
