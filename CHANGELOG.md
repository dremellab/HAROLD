...
- **STAR no longer pre-creates its own `--outTmpDir`:** STAR creates that directory itself and errors if it already exists; the rule's preceding `rm -rf` already guaranteed the path was absent, so the `mkdir -p` right before it was actively wrong, not just redundant.
